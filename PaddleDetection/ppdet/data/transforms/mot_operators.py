...
class LetterBoxResize(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if 'dif' in sample:
            dif=sample['dif']
            dif, _, _, _ = self.apply_image(
                dif, height=height, width=width)   
            sample['dif'] = dif         
        if 'flow' in sample:
            flow=sample['flow']
            flow, _, _, _ = self.apply_image(
                flow, height=height, width=width)   
            sample['flow'] = flow
        if 'depth' in sample:
            depth=sample['depth']
            depth, _, _, _ = self.apply_image(
                depth, height=height, width=width)   
            sample['depth'] = depth   
        if 'density' in sample:
            density=sample['density']
            density, _, _, _ = self.apply_image(
                density, height=height, width=width)   
...
class MOTRandomAffine(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if 'dif' in sample.keys():
            dif=sample['dif']
            difw=cv2.warpPerspective(
            dif,
            M,
            dsize=(width, height),
            flags=cv2.INTER_LINEAR,
            borderValue=self.borderValue)
            # difw = np.expand_dims(difw,axis=2)
       
        if 'depth' in sample.keys():
            depth=sample['depth']
            depthw=cv2.warpPerspective(
            depth,
            M,
            dsize=(width, height),
            flags=cv2.INTER_LINEAR,
            borderValue=self.borderValue)
            # depthw = np.expand_dims(depthw,axis=2)
            
        if 'flow' in sample.keys():
            flow=sample['flow']
            floww=cv2.warpPerspective(
            flow,
            M,
            dsize=(width, height),
            flags=cv2.INTER_LINEAR,
