...
class Decode_ms(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if 'dif' not in sample:
            dif_file=file.replace('.jpg','_dif.jpg')
            # with open(dif_file, 'rb') as f:
            #     sample['dif'] = f.read()
            dif = cv2.imread(dif_file)
            sample['dif'] = dif.astype(np.float32)
        if 'depth' not in sample:
            depth_file=file.replace('.jpg','_depth.jpg')
            # with open(depth_file, 'rb') as f:
            #     sample['depth'] = f.read() 
            depth=cv2.imread(depth_file)
            sample['depth']=depth.astype(np.float32)
        if 'flow' not in sample:
            flow_file=file.replace('.jpg','_flow.jpg')
            # with open(flow_file, 'rb') as f:
            #     sample['flow'] = f.read()
            flow=cv2.imread(flow_file)
            sample['flow']=flow.astype(np.float32)
        if 'density' not in sample:
            density_file=file.replace('.jpg','_den.jpg')
            # with open(density_file, 'rb') as f:
            #     sample['density'] = f.read()
            density=cv2.imread(density_file)
            sample['density']=density.astype(np.float32)
        im = sample['image']
        if 'h' not in sample:
            sample['h'] = im.shape[0]
        elif sample['h'] != im.shape[0]:
...
class RandomFlip(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            if 'dif' in sample and len(sample['dif'].shape)!=0:
                dif=sample['dif'] 
                dif = self.apply_image(dif)
                sample['dif']=dif
                
            if 'depth' in sample and len(sample['depth'].shape)!=0:
                depth=sample['depth'] 
                depth = self.apply_image(depth)
                sample['depth']=depth
                
            if 'flow' in sample and len(sample['flow'].shape)!=0:
                flow=sample['flow'] 
                flow = self.apply_image(flow)
                sample['flow']=flow                 

            if 'density' in sample and len(sample['density'].shape)!=0:
                density=sample['density'] 
                density = self.apply_image(density)
                sample['density']=density
            sample['flipped'] = True
            sample['image'] = im
        return sample
...
class Resize(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if len(im.shape) == 3:
            ...
            if 'dif' in sample:
                dif=sample['dif']
                sample['dif']=cv2.resize(dif,(w,h),interpolation=self.interp)
            if 'depth' in sample:
                depth=sample['depth']
                sample['depth']=cv2.resize(depth,(w,h),interpolation=self.interp)
            if 'flow' in sample:
                flow=sample['flow']
                sample['flow']=cv2.resize(flow,(w,h),interpolation=self.interp)
            if 'density' in sample:
                density=sample['density']
                sample['density']=cv2.resize(density,(w,h),interpolation=self.interp)       
        else:
            resized_images = []
            for one_im in im:
            ...
            if 'dif' in sample:
                ...
                re_difs=[]
                for dif in difs:
                    re_difs.append(cv2.resize(dif,(1440,800),interpolation=self.interp))
                sample['dif']=np.array(re_difs, dtype=np.float32)
                print(re_difs.shape)
            if 'depth' in sample:
                depths=sample['depth']
                re_depths=[]
                for depth in depths:
                    re_depths.append(cv2.resize(depth,(1440,800),interpolation=self.interp))
                sample['depth']=np.array(re_depths,dtype=np.float32)
            if 'flow' in sample:
                flows=sample['flow']
                re_flows=[]
                for flow in flows:
                    re_flows.append(cv2.resize(flow,(1440,800),interpolation=self.interp))
                sample['flow']=np.array(re_flows,dtype=np.float32)
            if 'density' in sample:
                densitys=sample['density']
                re_densitys=[]
                for density in densitys:
                    re_densitys.append(cv2.resize(density,(1440,800),interpolation=self.interp))
                sample['density']=np.array(re_densitys,dtype=np.float32)
