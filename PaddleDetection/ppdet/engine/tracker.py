...
MOT_ARCH = ['JDE', 'FairMOT', 'FairMOT_mifn', 'DeepSORT', 'ByteTrack', 'CenterTrack']
...
class Tracker(object):
        ...
        for seq in seqs:
            ...
            if not os.path.exists(infer_dir) or not os.path.isdir(infer_dir):
                logger.warning("Seq {} error, {} has no images.".format(
                    seq, infer_dir))
                continue
            if os.path.exists(os.path.join(infer_dir, 'img')):
                infer_dir = os.path.join(infer_dir, 'img')
    ...
    def get_infer_images(self, infer_dir):
        ...
        # images = list(images)
        # images.sort()       #使用MOT16数据集时再启用该部分代码
        images=[]
        for i in range(len(os.listdir(infer_dir))):
            images.append(infer_dir+'/'+str(i)+'.jpg')
    ...
    def mot_predict_seq(self,
        ...
        if video_file:
            ...
            if os.path.exists(os.path.join(image_dir, 'img')):
                image_dir = os.path.join(image_dir, 'img')
            # images = [
