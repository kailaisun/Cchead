...
DLA_mifn_cfg = {34: ([1, 1, 1, 2, 2, 1], [16, 32, 64, 128, 256, 512]),}


class BasicBlock(nn.Layer): #2个3×3卷积与BN层串联，不改变大小，带残差
...
@register
@serializable
class DLA_mifn(nn.Layer):
    """
    DLA, see https://arxiv.org/pdf/1707.06484.pdf

    Args:
        depth (int): DLA_mifn depth, should be 34.
        residual_root (bool): whether use a reidual layer in the root block

    """

    def __init__(self, depth=34, residual_root=False,pre_img=False,
                 pre_hm=False):
        super(DLA_mifn, self).__init__()
        levels, channels = DLA_mifn_cfg[depth]
        if depth == 34:
            block = BasicBlock
    ...
    def forward(self, inputs):
        ...
        for i in range(6):
            ...
            outs.append(feats)
        return outs, dif, flow, depth, density
