...
from paddle import ParamAttr, transpose
...
__all__ = ['CenterNetDLAFPN_mifn', 'CenterNetHarDNetFPN']
...
class Dif_attention(nn.Layer):
    def __init__(self):
        super(Dif_attention, self).__init__()
        self.conv1=ConvNormLayer(12,16,filter_size=3,stride=1,use_dcn=False,initializer=Uniform(-1,1))
        self.conv2=nn.Sequential(ConvNormLayer(16,32,filter_size=3,stride=1,use_dcn=False,initializer=Uniform(-1,1)),nn.ReLU())
        self.pool1=nn.AvgPool2D(kernel_size=2, stride=2)
        self.conv3=ConvNormLayer(32,64,filter_size=3,stride=1,use_dcn=False,initializer=Uniform(-1,1))
        self.conv4=nn.Sequential(ConvNormLayer(64,64,filter_size=3,stride=1,use_dcn=False,initializer=Uniform(-1,1)),nn.ReLU())
        self.pool2=nn.AvgPool2D(kernel_size=2, stride=2)
        self.fuse1=nn.Sequential(ConvNormLayer(128,128,filter_size=1,stride=1,use_dcn=False,initializer=Uniform(-1,1)),nn.ReLU(),
                                 ConvNormLayer(128,128,filter_size=1,stride=1,use_dcn=False,initializer=Uniform(-1,1)),nn.ReLU())
        self.fuse2=nn.Sequential(ConvNormLayer(128,128,filter_size=1,stride=1,use_dcn=False,initializer=Uniform(-1,1)),nn.ReLU(),
                            ConvNormLayer(128,128,filter_size=1,stride=1,use_dcn=False,initializer=Uniform(-1,1)),nn.ReLU())
        self.pool_h = nn.AdaptiveAvgPool2D((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2D((1, None))
        self.CA_conv1 = nn.Sequential(ConvNormLayer(128,32,filter_size=1,stride=1,use_dcn=False,initializer=Uniform(-1,1)),nn.Swish())
        self.sig=nn.Sigmoid()
        self.conv_h = nn.Conv2D(32,128,(1,1))
        self.conv_w = nn.Conv2D(32,128,(1,1))
        ...
        self.fuse_step2_1 = nn.Sequential(ConvNormLayer(64,64,filter_size=1,stride=1,use_dcn=False,initializer=Uniform(-1,1)),nn.ReLU())
        self.fuse_step2_2 = nn.Sequential(ConvNormLayer(64,64,filter_size=1,stride=1,use_dcn=False,initializer=Uniform(-1,1)),nn.ReLU())
        
    def CA_attention(self, x):
        identity = x
        n, c, h, w = x.shape
        x_h = self.pool_h(x)
        x_w = self.pool_w(x)
        x_w = transpose(x_w,perm=[0, 1, 3, 2])
        y = paddle.concat(x=[x_h, x_w], axis=2)
        y = self.CA_conv1(y)
        x_h, x_w = paddle.split(y, [h, w], axis=2)
        x_w = transpose(x_w,perm=[0, 1, 3, 2])
        a_h = self.sig(self.conv_h(x_h))
        a_w = self.sig(self.conv_w(x_w))
    ...
    def CBAM_attention(self,x):
        ...
        output = self.sig(max_out + avg_out)
        return x+x*output
    
    
    def forward(self, feat, dif, flow, depth, density):
        ms_feature = paddle.concat(x=[dif, flow, depth, density],axis=1)
        ms_feature=self.pool1(self.conv2(self.conv1(ms_feature)))
        ms_feature=self.pool2(self.conv4(self.conv3(ms_feature)))
        feat_cat=paddle.concat(x=[feat,ms_feature],axis=1)
        fuse=self.fuse1(feat_cat)
        fuse=self.CA_attention(fuse)
        fuse=self.CBAM_attention(fuse)
        fuse=self.fuse2(fuse)
        fuse=feat_cat*fuse+feat_cat
        feat=fuse[:,0:64,:,:]
        ms_feature=fuse[:,64:128,:,:]
        feat=self.fuse_step2_1(feat)
        ms_feature=self.fuse_step2_2(ms_feature)
        feat=feat+ms_feature*feat
        return feat
        

@register
@serializable
class CenterNetDLAFPN_mifn(nn.Layer):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 dcn_v2=True,
                 with_sge=False):
        super(CenterNetDLAFPN_mifn, self).__init__()
        ...
        self.dif_attention= Dif_attention()

    @classmethod
    def from_config(cls, cfg, input_shape):
        return {'in_channels': [i.channels for i in input_shape]}

    def forward(self, body_feats, dif, flow, depth, density):
        ...
        if self.down_ratio != 4:
            feat = F.interpolate(
                ...
                mode="bilinear",
                align_corners=True)
        feat=self.dif_attention(feat, dif, flow, depth, density)
        return feat
